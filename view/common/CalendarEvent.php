...
for(i=0;i<data.length;i++)
     {
     chartData.push({"title":data[i].title,"start":data[i].start_date,"end":data[i].end_date,"auditor":data[i].auditor,"auditee":data[i].auditee});
      }
  console.log(data);
$('#calendar').fullCalendar({
eventMouseover: function (data, event, view) {

            tooltip = '<div class="tooltiptopicevent" style="width:auto;height:auto;background:#a4a7ae;position:absolute;z-index:10001;padding:10px 10px 10px 10px ;  line-height: 200%;">' + 'Title: ' + ': ' + data.title + '</br>' + 'Start_date: ' + ': ' + data.start + '</br>'+ 'End_date: ' + ': ' + data.end + '</br>'+ 'Auditor: ' + ': ' + data.auditor + '</br>'+ 'Auditee: ' + ': ' + data.auditee + '</div>';
