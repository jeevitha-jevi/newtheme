...
$assetnotify=$manager->assetnotification();
$assetseennotify=$manager->setnotifyintoseen();
              ...
              <div class="portlet box red">
                ...
                <div class="portlet-body ">               
                  <div class="row" style="overflow-y: scroll; position:relative; height: 500px;">
                    ...
                    <?php
                     foreach($assetnotify as $asset){ ?>
                             <?php if($asset['notification_status']==1){ ?> 
                            <h5 style="padding: 25px; background-color: skyblue;">New <?php echo $asset['name']; ?> is <?php echo $asset['status']; ?> on <?php echo $asset['created_date']; ?> <?php echo $asset['notification_status']; ?></h5>
                            <?php } else { ?>
                              New <?php echo $asset['name']; ?> is <?php echo $asset['status']; ?> on <?php echo $asset['created_date']; ?>
                            <?php } ?>


                             <input type="hidden" name="submit" id="company" value="<?php echo $asset['id']?>">
                              <input type="hidden" name="submit" id="company_id" value="<?php echo $asset['company_id']?>">
                               <hr>
                  <?php }  ?>
                  <?php
                    setnotifyintoseen();
