...
$disasternotify=$manager->disasternotification();
$disasterseennotify=$manager->setnotifyintoseen();
              ...
              <div class="portlet box red">
                ...
                <div class="portlet-body ">               
                  <div class="row" style="overflow-y: scroll; position:relative; height: 500px;">
                    ...
                    <?php
                     foreach($disasternotify as $disaster){ ?>
                             <?php if($disaster['notification_status']==1){ ?> 
                            <h5 style="padding: 25px; background-color: skyblue;">New <?php echo $disaster['scope']; ?> is <?php echo $disaster['status']; ?></h5>
                            <?php } else { ?>
                              New <?php echo $disaster['scope']; ?> is <?php echo $disaster['status']; ?>
                            <?php } ?>


                             <input type="hidden" name="submit" id="company" value="<?php echo $disaster['id']?>">
                              <input type="hidden" name="submit" id="company_id" value="<?php echo $disaster['company_id']?>">
                               <hr>
                  <?php }  ?>
                  <?php
                    setnotifyintoseen();
