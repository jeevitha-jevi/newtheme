...
$risknotify=$manager->risknotification();
$riskseennotify=$manager->setnotifyintoseen();
              ...
              <div class="portlet box red">
                ...
                <div class="portlet-body ">               
                  <div class="row" style="overflow-y: scroll; position:relative; height: 500px;">
                    ...
                    <?php
                     foreach($risknotify as $risk){ ?>
                             <?php if($risk['notification_status']==1){ ?> 
                            <h5 style="padding: 25px; background-color: #B1EEDD;">New <?php echo $risk['subject']; ?> is <?php echo $risk['status']; ?> on <?php echo $risk['created_date']; ?></h5>
                            <?php } else { ?>
                              New <?php echo $risk['subject']; ?> is <?php echo $risk['status']; ?> on <?php echo $risk['created_date']; ?>
                            <?php } ?>


                             <input type="hidden" name="submit" id="company" value="<?php echo $risk['id']?>">
                              <input type="hidden" name="submit" id="company_id" value="<?php echo $risk['company_id']?>">
                               <hr>
                  <?php }  ?>
                  <?php
                  setnotifyintoseen();
