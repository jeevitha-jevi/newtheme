$(document).ready(function () {
    ...
    $('#modaldetails').on('click', 'tr', function(){
        if ($(this).hasClass('selected')){
             ...
             if(selectedData[0][6]=="assessment returned"){
    ...
    $.ajax({
        dataType: "json",
        type: "POST",
        url: "/freshgrc/php/asset/assetAssesmentList.php",
    ...
    //     dataType: "json",
    //     type: "POST",
    //     url: "/freshgrc/php/asset/assetAssessmentReturn.php",
    //     data: userCredentials,
    //     success: success
    // });
});
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        for (var colIndex = 0; colIndex < columns.length; colIndex++) {
            ...
            if (colIndex == 6 && "assessed" == auditstatus){
              cellValue="assessed";
              row$.append($('<td class="btn"  style="width:130px; height:50% ; background-color:#5cb85c; color:#fff; text-align:center;margin: 10px;"/>').html(cellValue));   
            }
              else if (colIndex == 6 && "assessment returned" == auditstatus){
              cellValue="assessment returned";
              row$.append($('<td class="btn" style="width:130px; height:50% ; background-color:#32c5d2; color:#fff; margin: 10px;"/>').html(cellValue));   
            }
...
function performAssetActions() {
    var selectedData = table.rows('.selected').data();
    if(selectedData[0][6]=="assessment returned"){
    window.location = "/freshgrc/view/asset/assetAction.php?assetId=" + selectedData[0][0];
}
