$(document).ready(function () {
    ...
    $.ajax({
        dataType: "json",
        type: "POST",
        url: "/freshgrc/php/compliance/complianceReportList.php",
...
function showComplClause() {
    var selectedData = table.rows('.selected').data();
    window.location = "/freshgrc/view/compliance/clauseReportAdmin.php?complianceId=" + selectedData[0][0];
}
function importCsv() {
    //var selectedData = table.rows('.selected').data();
...
function showComplClauseDashboard(){
  var selectedData = table.rows('.selected').data();
  window.location="/freshgrc/view/compliance/complianceDashboard.php?id="+selectedData[0][0];
}